 ...
 * @param {string} type - Le type de notification ('error', 'success', 'riposte').
 * @param {HTMLElement|null} [targetElement=null] - L'élément cible pour positionner la notification.
 */
...
/**
 * Crée un texte flottant pour les chiffres (dégâts, soins).
...
/**
 * NOUVEAU : Crée un texte flottant pour les événements spéciaux (mots).
 * @param {string} text - Le mot à afficher (ex: "CRITIQUE!").
 * @param {string} type - Le type de texte pour le style (ex: 'crit-text', 'defense-text').
 * @param {HTMLElement} targetElement - L'élément DOM au-dessus duquel le texte doit apparaître.
 */
export function createFlavorFloatingText(text, type, targetElement) {
    ...
    const rect = targetElement.getBoundingClientRect();
    const xOffset = rect.width * 0.5; // Centré
    textEl.style.left = `${rect.left + xOffset}px`;
    textEl.style.top = `${rect.top}px`;
    textEl.style.transform = 'translateX(-50%)';

    floatingTextContainerEl.appendChild(textEl);
    // Utilise une animation différente pour plus d'impact
