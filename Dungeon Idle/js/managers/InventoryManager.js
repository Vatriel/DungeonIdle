...
function equipItem(state, hero, item, inventoryIndex, eventBus) {
    ...
    // CORRIGÉ : On passe l'objet 'state' à la méthode equipItem du héros
    hero.equipItem(state, item, targetSlot);
...
function unequipItemFromHero(state, hero, slot, eventBus) {
    ...
    // CORRIGÉ : On passe l'objet 'state' à la méthode unequipItem du héros
    const itemToUnequip = hero.unequipItem(state, slot);
